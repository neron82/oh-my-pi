...
export class StatusLineComponent<TSession extends StatusLineSession = StatusLineSession> implements Component {
	...
	/**
	 * Rendered second line below the bar (tok/s · cache hit · session totals),
	 * derived from `SegmentContext.usageStats` on every real bar build. A cache
	 * hit reuses the value from the build that produced the cached bar, whose
	 * key covers the same usage inputs.
	 */
